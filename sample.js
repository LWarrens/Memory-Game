  ...
  Memory.prototype._gamePlayEvents = function(tile) {
    ...
    tile.addEventListener( "click", function(e) {
      if (!this.classList.contains("flipped")) {
		...
		// var currentTile= self.card1id
		localStorage.setItem('self.card1id',self.card1id);
		 // var currentWord= "barn"
		...
		showWord ()
		 ...
		 showWord ()
		  ...
		  setTimeout(cardTurn, 2);
		 // var currentTile= self.card2id
		// var currentWord= "dog"
			localStorage.setItem('self.card2id',self.card2id);
		// localStorage.setItem('currentWord',currentWord);
		 if (self.card2id == 1) {
			var currentWord = "bear"
		 localStorage.setItem('currentWord',currentWord);
